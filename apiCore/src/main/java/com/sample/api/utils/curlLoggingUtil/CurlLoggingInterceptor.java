package com.sample.api.utils.curlLoggingUtil;
    ...
    public void process(HttpRequest request, HttpContext context) throws HttpException, IOException {
        try {
            ...
            if(CurlLoggingInterceptor.captureAPIDetails) {
                com.sample.reporting.extentReports.Logger.logInfo(message.toString());
                log.debug(message.toString());
            }
        } catch (Exception e) {
            log.warn("Failed to generate CURL command for HTTP request", e);
            com.sample.reporting.extentReports.Logger.logInfo("Failed to generate CURL command for HTTP request");
        }
    }
